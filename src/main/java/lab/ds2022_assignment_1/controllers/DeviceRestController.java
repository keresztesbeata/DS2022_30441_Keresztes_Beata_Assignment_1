...
public class DeviceRestController {
    ...
    public DeviceDTO addDevice(@RequestBody @Valid DeviceData request) {
        return deviceService.addDevice(request);
    }

    @GetMapping(DEVICES_PATH)
    public List<DeviceDTO> getDevicesByAccountId(@RequestParam @ValidUUID String accountId) throws EntityNotFoundException {
    ...
    public DeviceDTO updateDevice(@PathVariable(DEVICE_ID) @ValidUUID String deviceId,
                                  @RequestBody @Valid DeviceData request) throws DuplicateDataException, EntityNotFoundException {
        return deviceService.updateDevice(deviceId, request);
    }

    @GetMapping(DEVICE_ID_PATH)
    public DeviceDTO getDeviceById(@PathVariable(DEVICE_ID) @ValidUUID String deviceId) throws EntityNotFoundException {
        return deviceService.findDeviceById(deviceId);
    }

    @DeleteMapping(DEVICE_ID_PATH)
    public void deleteDevice(@PathVariable(DEVICE_ID) @ValidUUID String deviceId) throws EntityNotFoundException {
        deviceService.deleteDevice(deviceId);
    }

    @PostMapping(value = LINK_DEVICE_PATH, consumes = APPLICATION_JSON_VALUE)
    public void linkDevice(@RequestBody @Valid LinkDeviceRequest request) throws DuplicateDataException, EntityNotFoundException {
